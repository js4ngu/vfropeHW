...
class dualPortCOSlut() extends Module {
    ...
    io.ENout      := ENReg
    io.cosOut     := Mux(ENReg, cosLUT.read(io.cosIndex), 0.U(32.W))
    io.sinOut     := Mux(ENReg, cosLUT.read(io.sinIndex), 0.U(32.W))
    io.xFWD(0)    := Mux(ENReg, x0FWD                  , 0.U(32.W))
    io.xFWD(1)    := Mux(ENReg, x1FWD                  , 0.U(32.W))
    io.signFWD(0) := Mux(ENReg, cosSign                , 0.U(32.W)) // cos
    io.signFWD(1) := Mux(ENReg, sinSign                , 0.U(32.W)) // sin
}

//내일 검토해보기
class multiPortCOSlut(N : Int) extends Module {
    val io = IO(new Bundle {
        val EN        = Input(Bool())
        val x         = Input(Vec(N, Vec(2, UInt(32.W))))
        val cosIndex  = Input(Vec(N, UInt(32.W)))
        val sinIndex  = Input(Vec(N, UInt(32.W)))
        val sign      = Input(Vec(N, Vec(2, Bool())))
        ...
        val signFWD   = Output(Vec(N, Vec(2, Bool())))
    })
    ...
    val ENReg      = RegInit(false.B)
    val xFWDReg    = RegInit(VecInit(Seq.fill(N)(VecInit(Seq.fill(2)(0.U(32.W))))))
    val signReg    = RegInit(VecInit(Seq.fill(N)(VecInit(Seq.fill(2)(false.B)))))

    ENReg := io.EN
    for (i <- 0 until N) {
        xFWDReg(i)(0) := io.x(i)(0)
        xFWDReg(i)(1) := io.x(i)(1)
        signReg(i)(0) := io.sign(i)(0)
        signReg(i)(1) := io.sign(i)(1)
    }

    io.ENout := ENReg

    for (i <- 0 until N) {
        io.cosOut(i)     := Mux(ENReg, cosLUT.read(io.cosIndex(i)), 0.U(32.W))
        io.sinOut(i)     := Mux(ENReg, cosLUT.read(io.sinIndex(i)), 0.U(32.W))
        io.xFWD(i)(0)    := Mux(ENReg, xFWDReg(i)(0), 0.U(32.W))
        io.xFWD(i)(1)    := Mux(ENReg, xFWDReg(i)(1), 0.U(32.W))
        io.signFWD(i)(0) := Mux(ENReg, signReg(i)(0), false.B) // cos
        io.signFWD(i)(1) := Mux(ENReg, signReg(i)(1), false.B) // sin
    }
}