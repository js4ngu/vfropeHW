...
class CosLUTbank2() extends Module {
    val io = IO(new Bundle {
        val x        = Input(Vec(2, UInt(32.W)))
        val EN        = Input(Bool())
        val cosIndex  = Input(UInt(32.W))
...
val cosLUTBank3 = VecInit(Seq(
    ...
    val sinSignReg     = RegInit(VecInit(Seq.fill(3)(false.B)))
    val cosSignReg     = RegInit(VecInit(Seq.fill(3)(false.B)))
    ENReg(0)      := io.EN
    sinSignReg(0) := io.sinSign
    cosSignReg(0) := io.cosSign
    ENReg(1)      := ENReg(0)
    sinSignReg(0) := sinSignReg(1)
    cosSignReg(0) := cosSignReg(1)
    ENReg(2)      := ENReg(1)
    sinSignReg(2) := sinSignReg(1)
    cosSignReg(2) := cosSignReg(1)

    val stage1Reg      = RegInit(VecInit(Seq.fill(4)(0.U(32.W))))
    val stage2Reg      = RegInit(VecInit(Seq.fill(3)(0.U(32.W))))
    val stage3Reg      = RegInit(VecInit(Seq.fill(6)(0.U(32.W))))
    
    //stage1
    stage1Reg(0) := io.cosIndex
    stage1Reg(1) := io.sinIndex
    stage1Reg(2) := io.x(0)
    stage1Reg(3) := io.x(1)

    //stage2
    stage2Reg(0) := cosLUT(stage1Reg(0))
    stage2Reg(1) := stage1Reg(2)
    stage2Reg(2) := stage1Reg(3)

    //stage3
    stage3Reg(0) := stage2Reg(0)
    stage3Reg(0) := cosLUT(stage1Reg(1))
    stage3Reg(2) := stage2Reg(1)
    stage3Reg(3) := stage2Reg(2)

    //output
    io.cosOut  := Mux(ENReg(2), stage3Reg(0), 0.U)
    io.sinOut  := Mux(ENReg(2), stage3Reg(1), 0.U)
    io.xFWD(0)  := Mux(ENReg(2), stage3Reg(2), 0.U)
    io.xFWD(1)  := Mux(ENReg(2), stage3Reg(3), 0.U)
    io.ENout    := ENReg(2)
