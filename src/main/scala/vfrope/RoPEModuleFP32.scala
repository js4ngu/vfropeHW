...
class multiLaneRoPEmodule(N: Int, Index: Int, LutSize: Int, LutHalfSizeHEX: Int, doublePi: Int, OneAndHalfPi: Int, Pi: Int, halfPi: Int) extends Module {
    val io = IO(new Bundle {
        val x           = Input(Vec(N, Vec(2, UInt(32.W))))
        val EN          = Input(Bool())
        val m           = Input(Vec(N, UInt(32.W)))
        val baseIndex   = Input(Vec(N, UInt(32.W)))
        val TwoDivD     = Input(Vec(N, UInt(32.W)))
    ...
    val RadCalc = Array.fill(N) {
        Module(new FP32radianCaclulator(LutSize, LutHalfSizeHEX, Index))
    }
    val SinCosLut = Module(new multiPortSinCosModule(N, LutSize, LutHalfSizeHEX, doublePi, OneAndHalfPi, Pi, halfPi))
    ...
    for(i <- 0 until N) {
        ...
        RadCalc(i).io.TwoDivD   := stage1Reg(i)(4)
    }
